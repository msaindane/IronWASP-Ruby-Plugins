...
class XHeaderAnalysis < PassivePlugin
    ...
    def report_technology_used(sess, results, tech_used)
    	...
    	if is_signature_unique(sess.request.host, PluginResultType.information, signature)
            plugin_result = PluginResult.new(sess.request.host)
    ...
    def report_xss_protection(sess, results, xss_protection)
        ...
        if is_signature_unique(sess.request.host, PluginResultType.information, signature) or is_signature_unique(sess.request.host, PluginResultType.vulnerability, signature)
            plugin_result = PluginResult.new(sess.request.host)
